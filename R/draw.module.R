...
#' which can be created with \code{\link[RCytoscape]{CytoscapeConnection}}.
#'
#' @import RCy3
#' @import data.table
#' @import igraph
...
draw.module<-function(moduleNR, graph, title="", nodes, colors, save.image=TRUE, cy){
  ...
  # 
  # 
  # g <- initEdgeAttribute (g, "Correlation", "numeric", 0.0)
  # g <- initEdgeAttribute (g, "weight", "numeric", 0.0)
  # g <- initEdgeAttribute (g, "p-Value", "numeric", 0.0)
  # cy = CytoscapeConnection ()
  # cw = new.CytoscapeWindow (paste0(title, "_", "module", moduleNR), g)
  # displayGraph(cw)
  # setNodeLabelRule (cw, "label")
  # setNodeSizeRule(cw, "p.value", c(0.05/vcount(graph), 0.05/vcount(graph)*0.01,0.05), c(80,60,45,30,20), mode = "interpolate")
